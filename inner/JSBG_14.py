...
xpu = XlsPosUtil()
def mean_ignore_special_values(series, special_value=-9999):
    return series[series != special_value].mean()
...
def statistics_all(df, yaml_data, wb, bar):
    ...
    pivot_df = pd.pivot_table(df, values=values, index=index, columns=columns, aggfunc=mean_ignore_special_values, fill_value=0, margins=True)

    sheet = get_sheet(wb, sheet_name)
    fill_title(sheet, title)
    fill_template(sheet, row_start_position, [each for each in pivot_df.columns])
    fill_template(sheet, col_start_position, [each for each in pivot_df.index], False)
    for i, each in enumerate(pivot_df.columns):
        result_position = xpu.position_add_col(result_start_position, i)
        fill_template(sheet, result_position, pivot_df[each], False)
    bar()
    return wb