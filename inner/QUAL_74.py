...
def statistics_all(df, csv_data, yaml_data, wb, bar, sheet_name="QUAL_74"):
    ...
    area_position_list, area_list, percent_list_position_list, percent_list = _get_result_list(df, result_start_position, street_value_list, group_field, group_field_value, calc_field, limit_field)
    sheet = get_sheet(wb, sheet_name)
    fill_title(sheet, title)
    fill_template(sheet, street_start_position, street_title_list, False, 1)
